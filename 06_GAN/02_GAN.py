## Generative Adversarial Network(GAN)을 구현한다.
## GAN은 Unsupervised Learning 중 하나이다.
...
n_hidden = 256
n_input = 28*28
# Generator의 입력값으로 사용할 노이즈의 크기
n_noise = 128


## 신경망 모델 구성
## GAN도 AutoEncoder 처럼 비지도 학습이므로 Y를 사용하지 않는다.
X = tf.placeholder(tf.float32, [None, n_input])
# 노이즈 Z를 입력 값으로 사용한다.
Z = tf.placeholder(tf.float32, [None, n_noise])


## Generator Neural Nets에서 사용하는 변수
...
# 판별기의 최종 결과 값이 얼마나 진짜와 가까운지 판단하는 한 개의 스칼라 값
...
## 논문에 따르면, GAN 모델의 최적화는 loss_G 와 loss_D 를 최대화 하는 것이다.
